...
const renderRoll = roll => {
  ...
  let output = Object.entries(roll)
    .map(([die, value]) => {
      let html = `
        <li class="dieItem ${die}Item"  data-die="${die}">
...
const disableInvalidChoices = () => {
  ...
  const rollList = document.querySelector('.rollDisplay');
  // rollList.addEventListener('click', e => {
  // let selectedDie = diceSet.toggleKeep(e.target.data-die)
  // console.log(e.target);
  // let selectedValue = e.target.value;

  keptDice.push(selectedValue)
